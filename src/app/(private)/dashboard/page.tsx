      ...
      <table className='w-full table-auto border-collapse border'>
        <thead>
          <tr className='bg-gray-100'>
            <th className='border px-2 text-center'>タイトル</th>
            <th className='border px-2 text-center'>表示 / 非表示</th>
            <th className='border px-2 text-center'>更新日時</th>
            <th className='border px-2 text-center'>操作</th>
          </tr>
        </thead>
        <tbody>
          {posts.map((post) => (
            <tr key={post.id}>
              <td className='border p-2'>{post.titled}</td>
